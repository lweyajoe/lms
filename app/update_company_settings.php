...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Get the new interest rate and interest billing period
    ...
    $sql = "REPLACE INTO company_settings 
            (id, company_name, company_address, company_email, company_website, company_phone, tax_rate, interest_rate, interest_billing_period, bank_name, account_number, account_reference, payee_name)
            VALUES (1, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

    if ($stmt = $conn->prepare($sql)) {
        // Bind parameters, changing 'tax_rate' to 'i' for integer
        $stmt->bind_param("sssssiisssss", $company_name, $company_address, $company_email, $company_website, $company_phone, $company_tax_rate, $interest_rate, $interest_billing_period, $bank_name, $account_number, $account_reference, $payee_name);
