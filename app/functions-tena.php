...
function sendEmail($to, $subject, $messageBody) {
    ...
    try {
        ...
        $mail->Host       = 'smtp.omabracredit.co.ke';                 // Set the SMTP server to send through
        $mail->SMTPAuth   = true;                                 // Enable SMTP authentication
        $mail->Username   = 'support@omabracredit.co.ke';                // SMTP username
        $mail->Password   = 'Invest@Growth2024!';                       // SMTP password
        $mail->SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS;       // Enable TLS encryption
        $mail->Port       = 587;                                  // TCP port to connect to

        // Recipients
        $mail->setFrom('support@omabracredit.co.ke', 'Omabra Credit');
...
function isLoanCleared($loan_id) {
    ...
    $sql_payments = "SELECT SUM(payment_amount) AS total_payments FROM payments WHERE loan_id = ?";
    $stmt_payments = $conn->prepare($sql_payments);
    $stmt_payments->bind_param("s", $loan_id);
    $stmt_payments->execute();
    $result_payments = $stmt_payments->get_result();
...
function getStageStatus($loan_id) {
    ...
    $stmt->execute();
    $result = $stmt->get_result()->fetch_assoc();
    return $result['stage_status'] ?: 'unknown';
}
