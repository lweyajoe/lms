...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_FILES['id_photo_front']) && $_FILES['id_photo_front']['error'] === UPLOAD_ERR_OK) {
        if ($_FILES['id_photo_front']['size'] <= 10 * 1024 * 1024) {
            ...
            $error = "Front ID photo exceeds the 10MB limit.";
        }
    }

    if (isset($_FILES['id_photo_back']) && $_FILES['id_photo_back']['error'] === UPLOAD_ERR_OK) {
        if ($_FILES['id_photo_back']['size'] <= 10 * 1024 * 1024) {
            ...
            $error = "Back ID photo exceeds the 10MB limit.";
        }
    }

    if (!isset($error)) {
        // Check if client already exists
        if (clientExists($conn, $email, $phone, $nationalId)) {
            ...
            $stmt = $conn->prepare("INSERT INTO clients (first_name, last_name, email, phone_number, county, town_centre,
                                    ...
                                    residence_nearest_road, date_of_onboarding, onboarding_officer, age, gender, next_of_kin_name,
                                    next_of_kin_phone_number, next_of_kin_relation)
                                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            $stmt->bind_param("sssssssssssssssssss", $firstName, $lastName, $email, $phone, $county, $townCentre,
                              $nationalId, $idPhotoFront, $idPhotoBack, $workActivity, $residenceBuilding, $residenceRoad,
                              $dateOfOnboarding, $onboardingOfficer, $age, $gender, $nextOfKinName, $nextOfKinPhone, $nextOfKinRelation);
                ...
                if ($result && $result->num_rows > 0) {
                    ...
                    if ($stmt) {
                        ...
                        if ($stmt->execute()) {
                            ...
                            $message .= "You can log in to your dashboard using your email and this temporary password.\n\n";
...
<?php require_once("head.php"); ?>
        ...
        <div class="row">
            ...
            <div class="col-md-4">
                <div class="form-group">
                    ...
                    <input type="file" class="form-control-file form-control height-auto" name="id_photo_front" required />
                </div>
            </div>
            <div class="col-md-4">
                <div class="form-group">
                    <label>Attach ID (BACK SIDE) :</label>
                    <input type="file" class="form-control-file form-control height-auto" name="id_photo_back" required />
                </div>
            </div>
        </div>
        <div class="row">
    ...
    <section>
        <div class="row">
            <p>Please click the "SUBMIT" button. Client should receive an email with their details and a temporary password. The client can log in to their dashboard using their National ID Number and their password.</p>
        </div>
